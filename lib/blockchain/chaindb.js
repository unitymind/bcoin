...
ChainDB.prototype.getRawCoin = co(function* getRawCoin(hash, index) {
  ...
  raw = yield this.db.get(layout.c(hash, index));
...
ChainDB.prototype.getCoin = co(function* getCoin(hash, index) {
  var coin = yield this.getRawCoin(hash, index);
...
ChainDB.prototype.hasCoins = co(function* hasCoins(hash) {
  var iter;
...
ChainDB.prototype.getCoinView = co(function* getCoinView(tx) {
  ...
  for (i = 0; i < tx.inputs.length; i++) {
    ...
    coin = yield this.getRawCoin(prevout.hash, prevout.index);
